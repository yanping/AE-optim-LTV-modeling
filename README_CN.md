...
```text
.
├── Makefile                     # 自动化命令入口 (setup, auth, split, run, report, test, mask, unmask)
├── config.yaml                  # 全局唯一定义文件 (GCP 凭据、演化超参、模型配比、数据配置)
├── requirements.txt             # 项目依赖包清单
├── README.md                    # 英文使用与技术说明文档 (English Manual)
├── README_CN.md                 # 中文使用与技术说明文档 (简体中文)
├── DESIGN.md                    # 详细架构与技术设计文档 (English)
├── DESIGN_CN.md                 # 详细架构与技术设计文档 (简体中文)
├── INSIGHT.md                   # 历史多模型探索与 Tweedie 数学机理 (English)
├── INSIGHT_CN.md                # 历史多模型探索与 Tweedie 数学机理 (简体中文)
├── pytest.ini                   # Pytest 运行配置
├── venv/                        # Python 3.11 独立虚拟环境
├── alpha_evolve/                # Google Cloud 官方 AlphaEvolve 客户端与控制器 (严格只读)
├── scripts/                     # 交付运维与脱敏工具
│   └── mask_credentials.py     # 自动化凭据脱敏与一键恢复工具
...
└── tests/                       # 自动化测试脚本目录
    ...
    ├── test_mask_credentials.py # 凭据脱敏、备份与恢复测试
