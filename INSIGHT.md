...
1. **LightGBM + Tweedie 复合泊松-伽马分布 (`objective='tweedie'`)**：
   - 数学假设：$Y \sim \text{Tweedie}(p, \mu, \phi)$，其中方差幂 $1 < p < 2$。
   - 理论上把观测值视为一个泊松过程产生的充值频次 $N \sim \text{Poisson}(\lambda)$，与每次充值金额遵循的伽马分布 $Z_i \sim \text{Gamma}(\alpha, \beta)$ 的复合随机变量 $Y = \sum_{i=1}^N Z_i$。
...
4. **两阶段跨栏模型 (Two-Stage Hurdle Model)**：
   - **阶段一 (Classifier)**：LightGBM 二分类器预测用户是否会在远期付费 $P(\text{LTV} > 0 \mid X)$（以 AUC 为优化指标）。
   - **阶段二 (Regressor)**：仅在正样本（历史付费用户）子集上训练 LightGBM 回归器，预测付费金额期望 $E[\text{LTV} \mid \text{LTV} > 0, X]$。
   - **最终预测**：$\hat{y} = P(\text{LTV} > 0 \mid X) \times E[\text{LTV} \mid \text{LTV} > 0, X]$。
...
Tweedie 分布的方差形式为 $\text{Var}(Y) = \phi \cdot \mu^p$：
